...
public class MercTicketManager
{
	...
	// Note: These values are configured per castle in Siege.ini (XxxMaxMercenaries)
	// Each castle has unique mercenary limits to balance siege mechanics
	// Formula pattern: approximately 10-20 per merc type based on castle size
	// @formatter:off
	...
	// Maximum mercenary tickets per castle
	// Values configured in Siege.ini (XxxMaxMercenaries property for each castle)
	// Current implementation: Fixed per castle (simple, stable, easily configurable)
	// Retail implementation: Based on clan level (level 5 = 30 max, level 7+ = 50 max)
	// Note: Current system provides better balance for Interlude private servers
	// where castle ownership and clan levels can vary significantly.
