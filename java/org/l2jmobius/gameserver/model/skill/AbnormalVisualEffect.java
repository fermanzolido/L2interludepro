...
public enum AbnormalVisualEffect
{
	...
	// Note: VP_KEEP has duplicate mask value with VP_UP (0x10000000)
	// This is a known issue in Interlude - the correct mask value is unknown
	// VP_KEEP is likely from later chronicles and not used in Interlude
	// Kept for compatibility but should not be used to avoid conflicts with VP_UP
	VP_KEEP(0x10000000, 0),
	// Special
	...
	/**
	 * Event visual effects (type 2).
	 * <b>Interlude limitation:</b> These effects are from later chronicles (High Five+)
	 * and are not supported by the Interlude client. The client does not recognize
	 * or render these visual effects. Included here for forward compatibility and
	 * to prevent compilation errors in skills/effects that reference them.
	 * <b>Behavior:</b> When applied, these effects are silently ignored by the client.
	 */
	BR_NONE(0x000000, 2),
