...
public class FuncAtkAccuracy extends AbstractFunction
{
	...
	public double calc(Creature effector, Creature effected, Skill skill, double initVal)
	{
		...
		// Base accuracy from template (from XML) + [Square(DEX)] * 6 + level + weapon hitbonus
		double value = initVal + effector.getTemplate().getBaseAccuracy() + (Math.sqrt(effector.getDEX()) * 6) + level;
		if (level > 77)
		{
			value += level - 76;
		}
