...
public class Sow extends AbstractEffect
{
	...
	public void onStart(Creature effector, Creature effected, Skill skill)
	{
		...
		// After seeding, mob should not aggro on the player.
		// Clear aggro list and set to idle to prevent re-aggro.
		if (target.isAttackable())
		{
			target.asAttackable().stopHating(player);
			if (target.asAttackable().getMostHated() == null)
			{
	...
	{
		// Note: Success chance formulas are based on research and may need adjustments.
		// Base success rates: 90% for normal seeds, 20% for alternative seeds.
		...
		// Reduce success based on seed level vs target level difference
		...
		// 5% decrease in chance if player level is more than +/- 5 levels to target's level
		...
		// Ensure chance is at least 1% and at most 99%
		basicSuccess = Math.max(1, Math.min(99, basicSuccess));
