...
public class SummonNpc extends AbstractEffect
{
	...
	public void onStart(Creature effector, Creature effected, Skill skill)
	{
		...
		switch (npcTemplate.getType())
		{
			...
			case "EffectPoint": // Signet skills (ground-based channeling effects)
			{
				// Create the EffectPoint NPC that represents the signet on the ground
				final EffectPoint effectPoint = new EffectPoint(npcTemplate, player);
				effectPoint.fullRestore();
				
				// Determine spawn location (use ground target if available, otherwise player position)
				int x = player.getX();
				int y = player.getY();
				int z = player.getZ();
				...
				// Configure and spawn the effect point
				effectPoint.setInvul(true);
				effectPoint.setSummoner(player);
				effectPoint.spawnMe(x, y, z);
				
				// Schedule despawn based on NPC template configuration
				...
				// Note: Channeling is handled automatically by SkillChannelizer in Creature.java
				// The skill's channeling parameters define the periodic effect application
				break;
			}
			default:
			{
