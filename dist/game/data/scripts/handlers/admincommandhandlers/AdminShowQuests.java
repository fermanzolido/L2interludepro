...
/**
 * Admin command handler for managing player quests.
 * Provides interface to view, modify and manage quest states and variables.
 * Note: Improved security and code quality. Further refactoring may be beneficial.
...
public class AdminShowQuests implements IAdminCommandHandler
{
	...
	private void showQuestMenu(Player target, Player actor, String[] val)
	{
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			switch (val[0])
			{
				...
				case "name":
				{
					...
					final String state = (qs != null) ? STATES[qs.getState()] : "CREATED";
		...
		{
			actor.sendMessage("Error displaying quest information: " + e.getMessage());
			LOGGER.warning(AdminShowQuests.class.getSimpleName() + ": Error in showQuestMenu: " + e.getMessage());
			e.printStackTrace();
		}
	}
