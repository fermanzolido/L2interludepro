...
// 			@formatter:on
	...
	private void buildMessengerMessage()
	{
		...
		if (html != null)
		{
			// Store processed HTML - objectId will be set when sending to player
			_messengerHtml = html.replace("%nextSiege%", TimeUtil.formatDate(_hall.getSiegeDate().getTime(), "yyyy-MM-dd HH:mm:ss"));
		}
	}
	...
	{
		if (_messengerHtml != null)
		{
			final NpcHtmlMessage msg = new NpcHtmlMessage(npc.getObjectId(), _messengerHtml);
