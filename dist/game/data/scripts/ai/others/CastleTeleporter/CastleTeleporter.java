...
public class CastleTeleporter extends AbstractNpcAI
{
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("TELEPORT_EXECUTE"))
		{
			...
			// Optimized: Get players from surrounding WorldRegions instead of all players
			final WorldRegion npcRegion = npc.getWorldRegion();
			if (npcRegion != null)
			{
				final WorldRegion[] surroundingRegions = npcRegion.getSurroundingRegions();
				for (WorldRegion worldRegion : surroundingRegions)
				{
					for (WorldObject obj : worldRegion.getVisibleObjects())
					{
						if (obj.isPlayer())
						{
							final Player pl = obj.asPlayer();
