...
public class Chests extends AbstractNpcAI
{
	...
	private static final SkillHolder CHEST_SELF_BUFF = new SkillHolder(4245, 1); // Self-buff for mimics
		...
		if (npc instanceof Chest)
		{
			...
			if (!chest.isInteracted())
			{
				...
				else
				{
					...
					// Apply self-buff with random chance (50%)
					if (getRandomBoolean() && !chest.isSkillDisabled(CHEST_SELF_BUFF.getSkill()))
					{
						chest.setTarget(chest);
						chest.doCast(CHEST_SELF_BUFF.getSkill());
					}
