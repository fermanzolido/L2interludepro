		...
		switch (st.nextToken())
		{
			...
			case "manage_tax":
			{
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_VAULT))
				{
					...
					else
					{
						...
						html.replace("%next_tax_rate%", Integer.toString(castle.getTaxPercent())); // Current tax rate (will apply tomorrow if not changed)
			...
			case "manage_vault":
			{
				if (isOwner(player, npc) && player.hasAccess(ClanAccess.CASTLE_VAULT))
				{
					...
					html.replace("%tax_income_reserved%", "0"); // Note: Tax income reserved not tracked separately in current implementation
