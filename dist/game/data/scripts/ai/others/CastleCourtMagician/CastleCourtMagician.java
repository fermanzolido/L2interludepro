		...
		switch (event)
		{
			...
			case "clanTeleport":
			{
				if (player.getClanId() == npc.getCastle().getOwnerId())
				{
					...
					{
						if (CallPc.checkSummonTargetStatus(player, clanLeader)) // Custom check: retail does not check it but we check same conditions as when summoning player by skill
						{
